...
public:
    ...
    void load_program(const std::vector<uint16_t>& program, uint16_t start_address = USER_SPACE_ADDR) {
    ...
    uint16_t get_memory(uint16_t address) const {
    ...
    std::tuple<uint8_t, uint8_t, uint8_t> get_condition_codes() const {
        return std::make_tuple(condition_codes[0], condition_codes[1], condition_codes[2]);
    }
...
private:
    ...
    void execute_br(uint16_t instruction) {
        ...
        if ((n && condition_codes[0]) || (z && condition_codes[1]) || (p && condition_codes[2])) {
    ...
    void execute_trap(uint16_t instruction) {
        ...
        switch(trap_vector) {
            case HALT:
                ...
                break;
            case OUT:
                // For testing, we'll just store the character in a special memory location
...
PYBIND11_MODULE(lc3_simulator, m) {
    ...
    py::class_<LC3Simulator>(m, "LC3Simulator")
        ...
        .def("load_program", &LC3Simulator::load_program, py::arg("program"), py::arg("start_address") = USER_SPACE_ADDR)
