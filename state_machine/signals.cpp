...
{
    // ACV is set if:
    // 1. Accessing system space (< USER_SPACE_ADDR) while in user mode (PSR_15 = 0)
...
void SET_BEN()
{
    ...
    uint8_t n_bit = (instruction_reg & bit_table[11]) ? 1 : 0;  // Bit 11: branch if negative
    uint8_t z_bit = (instruction_reg & bit_table[10]) ? 1 : 0;  // Bit 10: branch if zero  
    uint8_t p_bit = (instruction_reg & bit_table[9]) ? 1 : 0;   // Bit 9: branch if positive
